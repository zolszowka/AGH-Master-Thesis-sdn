...
class MplsControllerBasic(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        super(MplsControllerBasic, self).__init__(*args, **kwargs)
    ...
    def _install_new_path(
        ...
        old_path_data = self.path_cache.get((src_dpid, dst_dpid))
        ...
        if old_path_data:
            self.logger.info(f"Expiring old label {old_path_data['label']}")
    ...
    def _expire_old_label(self, old_label: int, old_path: List[int]) -> None:
        for i in range(len(old_path) - 2, 0, -1):
            dpid = old_path[i]
